...
class Crc32 {
  ...
  append(data) {
    ...
    for (let offset = 0, len = data.length | 0; offset < len; offset++) {
      crc = (crc >>> 8) ^ _crc32Table[(crc ^ data[offset]) & 0xff];
    }
    this.crc = crc;
  }
...
class Entry {
  ...
  async text() {
    try {
      return await new Response(this.stream()).text();
    } catch (e) {
      throw new Error(`Failed to read Entry\n${e}`);
    }
  }
  ...
  async file() {
    try {
      return new File([await this.blob()], this.name, { lastModified: this.lastModified });
    } catch (e) {
      throw new Error(`Failed to read Entry\n${e}`);
    }
  }
...
function getBigInt64(view, position, littleEndian = false) {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    value += BigInt(byte) * 256**BigInt(i);
  }
